...
(5am:test lfu-replacement-policy
  (with-testing-cache (cache 100 :policy :lfu)
    (let* ((a (fetch-and-release cache 21))
	   (b (fetch-and-release cache 22))
	   (c (fetch-and-release cache 23))
	   (d (fetch-and-release cache 24)))
      (5am:is (= 0 (count-if #'cleaned-up-p (list a b c d))))
      (fetch-and-release cache 21)
      (fetch-and-release cache 23)
      (fetch-and-release cache 24)
      (fetch-and-release cache 23)
      (fetch-and-release cache 21)
      (let ((e (fetch-and-release cache 25)))
	(5am:is (cleaned-up-p b))
	(fetch-and-release cache 26)
...
(5am:test lfuda-replacement-policy
  (with-testing-cache (cache 100 :policy :lfuda :item-size-modulus 100)
    (let* ((a (fetch-and-release cache 125))
	   (b (fetch-and-release cache 225))
	   (c (fetch-and-release cache 325))
	   (d (fetch-and-release cache 425)))
      (dotimes (i 40)
	(5am:is (eq a (fetch-and-release cache 125)))
	(when (>= i 10)
	  (5am:is (eq d (fetch-and-release cache 425))))
	(when (>= i 20)
	  (5am:is (eq b (fetch-and-release cache 225))))
	(when (>= i 30)
	  (5am:is (eq c (fetch-and-release cache 325)))))
      (dotimes (i 5)
	(let ((e (fetch-and-release cache 525)))
	  (5am:is (cleaned-up-p c))
	  (setf c (fetch-and-release cache 625))
	  (5am:is (cleaned-up-p e))))
      (dotimes (i 100)
	(fetch-and-release cache 525)
	(fetch-and-release cache 625)
	(fetch-and-release cache 725)
	(fetch-and-release cache 825))
