...
(5am:test fifo-replacement-policy
  (with-testing-cache (cache 100 :policy :fifo)
    (let* ((a (fetch-and-release cache 20))
	   (b (fetch-and-release cache 30))
	   (c (fetch-and-release cache 40))
	   (d (fetch-and-release cache 25)))
      (5am:is (cleaned-up-p a))
      (5am:is (not (cleaned-up-p b)))
      (5am:is (not (cleaned-up-p c)))
      (5am:is (eq d (fetch-and-release cache 25)))
      (5am:is (eq c (fetch-and-release cache 40)))
      (5am:is (eq b (fetch-and-release cache 30)))
      (fetch-and-release cache 35)
...
(5am:test lifo-replacement-policy
  (with-testing-cache (cache 100 :policy :lifo)
    (let* ((a (fetch-and-release cache 20))
      ...
      (5am:is (eq a (fetch-and-release cache 20)))
      (5am:is (eq b (fetch-and-release cache 30)))
      (5am:is (eq d (fetch-and-release cache 25)))
      (fetch-and-release cache 35)
